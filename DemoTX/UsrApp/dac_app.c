...
// APB1 MAX 42Mhz
static void bsp_time1_init(void)
{
...
void bsp_dac_init(void)
{
    ...
    hdma_dac1.Instance = DMA1_Channel2;
    hdma_dac1.Init.Direction = DMA_MEMORY_TO_PERIPH;
	hdma_dac1.Init.Request = DMA_REQUEST_DAC_OUT1;
    ...
    if (HAL_DMA_ConfigChannelAttributes(&hdma_dac1, DMA_CHANNEL_NPRIV) != HAL_OK)
    {
      Error_Handler();
    }
...
static void dac1_initfor_dma(uint32_t buffer_addr, uint32_t count, uint32_t freq)
{
	...
	if(freq < 100)
	{
		htim1.Init.Prescaler = 10000 - 1; // 分频比=10000
		htim1.Init.Period = (tim_clk / 10000) / freq - 1; // 自动重装载值
	}
	else if(freq < 3000)
	{
		htim1.Init.Prescaler = 100 - 1; // 分频比=100
		htim1.Init.Period = (tim_clk / 100) / freq - 1; // 自动重装载值
	}
	else // 大于4k的频率，无需分频
	{
		htim1.Init.Prescaler = 0;
		htim1.Init.Period = tim_clk / freq - 1; // 自动重装载值
	}
